...
pub async fn route(
    ...
    while let Ok(Some(field)) = multipart.next_field().await {
        let original_file_name = field.file_name()
        ...
        let file_name = state.config.retain_uploaded_file_name
            .then_some(original_file_name.clone())
            .or(nanoid!(len).into());

        let is_public = query_param.public
            .unwrap_or(state.config.default_public) as u16;
        ...
        let file = sqlx::query_as!(
            ...
            "#,
            is_public, file_name, fp,
            content_type, file_hash, file_ext,
            original_file_name, query_param.folder
        )
